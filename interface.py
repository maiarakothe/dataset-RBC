...
from main import df, encontrar_similares, reutilizar, reter_caso
...
if not resultado.empty:
    ...
    filme = df[df["Series_Title"] == st.session_state.filme_escolhido].iloc[0]
    ...
    if st.session_state.recomendacao is None:
        ...
        st.session_state.recomendacao = reutilizar(resultado)
    ...
    percentual_recomendacao = recomendacao["Similaridade"] * 100

    st.write(f"Similaridade: " f"**{percentual_recomendacao:.2f}%**")

    # 3. REVISÃO
    # Com base na recomendação feita pelo sistema, o usuário indica se achou adequada ou não.
    st.subheader("📝 Revisão da recomendação")
    ...
    if st.button("Registrar avaliação", use_container_width=True):
        ...
        if not st.session_state.avaliado:
            ...
            st.info("Esta recomendação já foi avaliada.")
    ...
    st.info("Escolha um filme e clique em " "'Encontrar filmes similares'.")
